...
def score(success, max_iter, seed, maxidx):
    ...
    return out, sum(1 for v in S.successes.values() if v > 0)
...
def main():
    ...
    ap.add_argument("--maxidx", type=int, default=12,
                    help="restrict to functional samples among indices [0,maxidx)")
    args = ap.parse_args()
    ...
    for method in ("SLSQP", "trust-constr"):
        ...
        t0 = time.time()
        results[method], nsamp = score(args.success, args.max_iter, args.seed, args.maxidx)
        print(f"[{method}] done in {time.time()-t0:.0f}s ({nsamp} samples)", flush=True)
    ...
    print("\nNote: subset of samples for A/B speed; compares optimizers head-to-head, "
          "not the full-basin mean.")
